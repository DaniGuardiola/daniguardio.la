...
The original purpose of this experiment was completing a simple technical test requested by a company for a software engineering position, but I got carried away and ended up expanding the scope of the project a lot.

## Goal

The goal was to check if a user-provided password is included in a [list of most used passwords](https://github.com/danielmiessler/SecLists/blob/master/Passwords/xato-net-10-million-passwords.txt). The file with the passwords is 46.1 MB big, which obviously makes it difficult to process synchronously. Even the original test challenge pointed to the right solution: streams.
...
Now, the chunk size, at least in the case of Node.js, is fixed. That means that every chunk is the same size, no matter the content. The file content, however, is not organized as evenly, as every password is on a different line (utf8 encoding, newlines are the `\n` character). This means that what we really need is not to read chunk by chunk, but line by line, but streams don't support that natively.

The original challenge only required one solution. I have personally solved this problem before in Python, working with raw TCP connections that sent and received data with custom delimiters (if I remember correctly, TLS transports don't support EOF which complicates things... good times), so my initial intentions were to just send a solution and go about my day, but I wasn't satisfied after I finished, so I ended up spending my Sunday afternoon working on this.

Here's 4 different ways to read a big file in Node.js line-by-line.

## Methods

I am gonna explain these methods in an order that makes it easier to understand, but the original order in which I coded them was actually different (#2, #3, #1 then #4 if you must know).

You can check the full source code in the [`password_checkers.js`](password_checkers.js) file.

All benchmarks are calculated with a fast consumer-grade CPU (Intel(R) Core(TM) i7-9750H CPU @ 2.60GHz) and a solid-state drive. The comparison part of the tests is exactly the same and the read stream is obtained in the exact same way as well. All tests have the same format: an asynchronous function that accepts a user password and a stream chunk size, and (asynchronously) returns whether there's been a match or not (a boolean value).
...
Besides the big memory footprint, this also has the disadvantage of having to process the whole file, loading it into memory byte-per-byte and then looking for the `\n` character byte-per-byte. That's already two loops, and it also means that you have both the full file text in a single string and the same content but split into multiple string of an array.
...
**Password in the line 100:** `taylor`
...
**Password in the line 2594727 (halfway):** `gint3333`
...
**Password in the line 5189454 (last one):** `!!!!!!55`
...
> Note: when memory is not critical, a variation of this method could actually be a serious contender for speed. The time-expensive task here is loading the file into memory and processing it to split it. In a service scenario this would be an acceptable setup time, and the memory consumption could be a good trade-off. Having all passwords loaded into memory would very likely result in a faster lookup than loading them from the filesystem each time (specially with old hark disk drives).

### Method #2: ReadStream

From now on, we're only gonna work with streams. The most straightforward way to get a stream for a file is to use the [`fs.createReadStream()`](https://nodejs.org/docs/latest/api/fs.html#fs_fs_createreadstream_path_options) method. This return a [`fs.ReadStream`](https://nodejs.org/docs/latest/api/fs.html#fs_class_fs_readstream) instance, which is a readable stream.
...
This is fine because if a chunk actually ends in a newline character (if the last line is not a fragment of a password), splitting it will cause the last element to be an empty string, so when we add it to the next chunk it won't have any effect in the next element:
...
The last detail would be to make sure that we're checking the last line of the last chunk when there's no more chunks to process:
...
Another thing I try to do is always looks for "native" libs from the language or runtime first. Be it the standard library from Python or the APIs from the Node.js runtime. It simplifies the codebase and guarantees a very good level of support.
...
This method is still far more efficient in both compute time and memory usage than method #1, but seems a bit slower and memory-hungrier than method #2. It's more abstract and easier to implement too, and that could be a good trade-off, specially considering how relatively small the difference is.
...
### Password in the line 2594727 (halfway)
...
### Password in the line 5189454 (last one)
...
Memory stays about the same, method #2 is the fastest once again, and method #4... well...

## Conclusion

Stick to method #2 or #3. Method #3 is more convenient and method #2 is slightly faster, but both are good options.

These tests would be more accurate if they were run multiple times to get averages, but I feel that this is good enough. Specially considering I really could just have settled with sending the method #2 solution to the problem I was originally presented, which would've taken me about 20 minutes. But I have no regrets, this was fun. Yes, I have a weird definition of fun. Peace out!
